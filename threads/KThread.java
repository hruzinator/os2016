...
public class KThread {
	...
	Machine.autoGrader().readyThread(this);
        ...
        if(joiningThread != null){
            Lib.debug(dbgThread, "Warning! A KThread tried to join a thread that already"
                    + " has a joining thread attached to it. Can't join the new thread");
            return;
        }
        
        boolean intStatus = Machine.interrupt().disable();
        joiningThread = currentThread;
        currentThread.sleep();
        Machine.interrupt().restore(intStatus);
    }

    public static void joinSelfTest() {
        Lib.debug(dbgThread, "Enter KThread.joinSelfTest");

        p = new KThread(new Runnable() {
            @Override
            public void run() {
                for(int x = 1; x <= 5; x++) {
                    System.out.println("Thread P on iteration: " + x);
                    if(x == 3) {
                        q.fork();
                        q.join();
                    }
        ...
        q = new KThread(new Runnable() {
            ...
            public void run() {
                for(int x = 1; x <= 5; x++) {
                    System.out.println("Thread Q on iteration: " + x);
                    currentThread.yield();
                }
            }
        });
    ...
    /** If join is called on this KThread, this variable will store the KThread */
    private KThread joiningThread = null;
